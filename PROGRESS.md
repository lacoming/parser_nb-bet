...
| 13 | UI окно + трей UX | ✅ DONE | 2026-02-21 |
...
## Шаг 13 — DONE (2026-02-21)
**Задача:** UI окно + трей UX (1:1 по требованиям).

**Сделано:**
- `Ui/MainForm.cs` — полная переработка UI:
  - Размер окна увеличен: 600×520, resizable (min 500×420)
  - Status panel (top): заголовок, строка следующего запуска, строка последнего результата
  - Log viewer (center): тёмный TextBox (Consolas 8.5pt) с автоскроллом, лимит 500 строк
  - Button panel (bottom): "Запустить", "Пауза", "Скрыть в трей", "Выход"
  - Pause/Resume: останавливает и перезапускает MskScheduler
  - AppendLog(string) — thread-safe добавление строки в лог-просмотрщик
- `Logging/UiLogSink.cs` — кастомный Serilog sink:
  - Принимает Action<string> callback
  - Форматирует: `HH:mm:ss [LVL] message` + exception info
  - Используется MainForm для отображения логов в реальном времени
- `Logging/LoggingSetup.cs` — обновлён:
  - Принимает опциональный UiLogSink для UI-режима
- `Program.cs` — переработан для UI-режима:
  - UI sink с буферизацией: логи до создания формы буферизуются в ConcurrentQueue, затем drain
  - Чёткое разделение UI-пути и headless-пути
- Tray menu расширено: "Открыть окно", "Запустить сейчас", "Пауза/Возобновить", "Открыть логи", "Выход"
- "Открыть логи" — открывает папку logs/ в Explorer

**UX (1:1 по требованиям):**
- На старте: окно видно ✅
- Кнопка "Скрыть в трей" → hide + tray balloon ✅
- X → диалог "Выйти или Свернуть?" ✅
- Tray: двойной клик → открыть окно ✅
- Tray menu: "Открыть окно", "Запустить сейчас", "Пауза", "Открыть логи", "Выход" ✅

**Проверки:**
- `dotnet build --configuration Release` ✅ (0 ошибок, 0 предупреждений)
- `dotnet test` ✅ (165/165 passed — без регрессий)

**Follow-ups:**
- Шаг 14: E2E прогон + упаковка в .exe
- Добавить icon.ico (заказчик предоставит или сгенерируем)
